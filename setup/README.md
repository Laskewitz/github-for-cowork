...
action. To publish manually instead, if the manifest version is `1.0.0`, run
these commands from a checkout at the intended release commit after confirming
that `v1.0.0` does not already exist:

```sh
git tag -a v1.0.0 -m "GitHub for Cowork 1.0.0"
...
You can also trigger the same workflow manually: open the repository's **Actions**
tab, select **Release plugin package**, and click **Run workflow**.
